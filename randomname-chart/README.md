# Helm Chart Templates for Random Name Service in Kubernetes
...
Using the Helm charts assumes the following pre-requisites are complete:  
...
3. You have the Helm command line installed  
   [Helm](https://helm.sh/docs/intro/using_helm/) provide the command line tool for deploying this service using the Helm chart.
...
| `global.autoscaling.targetCPUUtilizationPercentage`    | Target CPU utilization percentafe to scale the services                                    | `80`                         |
| `global.autoscaling.targetMemoryUtilizationPercentage` | Target Memory utilization percentafe to scale the services                                 | `80`                         |
| `global.replicaCount`                                  | If the autoscaling is enabled, then this configuration will have no effects                | `1`                          |
| `global.backend.serviceName`                           | Kubernetes object service name to be refered by frontend service to access backend service | `randomname-backend-service` |
...
| `backend.image.tag`                  | Overrides the image tag whose default is the chart appVersion.                                                         | `latest`       |
...
| Parameter                             | Description                                                                                                            | Default                               |
|---------------------------------------|------------------------------------------------------------------------------------------------------------------------|---------------------------------------|
| `frontend.image.repository`           | Image repository for frontend service                                                                                  | `backend`                             |
...
| `frontend.ingress.hosts.paths`        | Path for this ingress to be routed to                                                                                  | `/`                                   |
...
1. From this project directory, run:  
   ...
   TEST SUITE: None
   ```
   You can specify which environment you want to deploy this service with specifying `--namespace namespace`.

   To verify this service is running, run the following two commands
   ```bash
...
* Find the deployment using `helm list --all` and searching for an entry with the chart name "`release-name`".
